...
let getBufferFromAccessor = (gltfRuntime, accessor) => {
    ...
    let arrayBuffer = gltfRuntime.arrayBuffers[bufferView.buffer];
    ...
    for (let i=0; i < gltfMesh.primitives.length; i++) {
      ...
      for (let semantic in gltfPrimitive.attributes) {
        ...
        if (semantic === "NORMAL") {
          ...
          for (let uv = 0; uv < buffer.length; uv++) {
            vertexBuffer.uvs[uv] *= 10;
          }
        }
      }
...
let buffersLoaded = (gltfRuntime, device) => {
  ...
  let loadBuffer = (bufferKey) => {
    return () => {
      ...
      if (Core.isBase64(gltfBuffer.uri)) {
        gltfRuntime.arrayBuffers[bufferKey] = Core.decodeArrayBuffer(gltfBuffer.uri.split(',')[1]);
        loadedBuffersCount++;
        ...
        Core.LoadFile(rootUrl + gltfBuffer.uri, true, (data) => {
          ...
          gltfRuntime.arrayBuffers[bufferKey] = data;
          loadedBuffersCount++;
